...
# 预测函数
def predict(image_data, model_name):
    ...
    model.eval()
    image = image.convert('RGB') # Now 'image' is a PIL Image and convert will work
...
# Gradio界面 with gr.ImageEditor
image_input = gr.ImageEditor(type="pil", label="Input Image") # Changed to gr.ImageEditor
model_input = gr.Radio(choices=["ResNet", "GoogLeNet","FastCNN"], label="Select Model")
output = gr.Label(num_top_classes=6)


gr.Interface(fn=predict, inputs=[image_input, model_input], outputs=output, 
             title="Emotion Prediction", 
             description="Upload or edit an image and select a model to predict the emotion.").launch() # Slightly improved description